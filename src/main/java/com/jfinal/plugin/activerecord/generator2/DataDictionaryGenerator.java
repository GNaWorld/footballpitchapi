...
public class DataDictionaryGenerator {
	...
	public DataDictionaryGenerator(DataSource dataSource, String 
...
dataDictionaryOutputDir) {
	...
	public void generate(List<TableMeta> tableMetas) {
		...
		System.out.println("Data Dictionary Output Dir: " + 
...
dataDictionaryOutputDir);
	...
	*/
	protected void genCell(int columnMaxLen, String preChar, String value, String 

fillChar, String postChar, StringBuilder ret) {
		ret.append(preChar);
		ret.append(value);
	...
	protected void genColumn(TableMeta tableMeta, ColumnMeta columnMeta, StringBuilder 
	...
	protected void rebuildColumnMetas(List<TableMeta> tableMetas) {
		...
		try {
			...
			for (TableMeta tableMeta : tableMetas) {
				...
				ResultSet rs = dbMeta.getColumns(conn.getCatalog(), null, 
...
+ "," + decimalDigits;
					...
					columnMeta.isNullable = rs.getString
...

{
					...
					columnMeta.defaultValue = rs.getString
...
("COLUMN_DEF");	// 默认值
	...
	// 备注
					...
					if (tableMeta.colNameMaxLen < 

columnMeta.name.length()) {
						tableMeta.colNameMaxLen = 

columnMeta.name.length();
					}
					if (tableMeta.colTypeMaxLen < 

columnMeta.type.length()) {
						tableMeta.colTypeMaxLen = 

columnMeta.type.length();
					}
					if (tableMeta.colDefaultValueMaxLen < 

columnMeta.defaultValue.length()) {
						tableMeta.colDefaultValueMaxLen = 
...
columnMeta.defaultValue.length();
			...
			if (conn != null) {
				try {conn.close();} catch (SQLException e) {LogKit.error
...
(e.getMessage(), e);}
	...
	protected void writeToFile(String ret) {
		...
		try {
			...
			String target = dataDictionaryOutputDir + File.separator + 
...
dataDictionaryFileName;
			...
			if (fw != null) {
				try {fw.close();} catch (IOException e) {LogKit.error
